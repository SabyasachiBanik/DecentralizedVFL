...
def train(models, device, train_loader, optimizers, epoch, input_size):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        for i, model in enumerate(models):
            ...
            optimizer.zero_grad()
            output = model.predict(combined_hidden_outputs) # Important: Like MNIST, here too, the local prediction is performed after receiving the output of other participants' hidden layer for Prototype 2
...
for seed in range(num_seeds):
    ...
    for num_participants in range(2, 11):  # 2 to 10 participants
        ...
        hidden_size = 10  # Unlike MNIST, we use 1 HL instead of 3
        output_size = 10

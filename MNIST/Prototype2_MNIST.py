...
def train(models, device, train_loader, optimizers, epoch, input_size):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        for i, model in enumerate(models):
            ...
            optimizer.zero_grad()
            output = model.predict(combined_hidden_outputs) # Important: The local prediction is performed after receiving the output of other participants' hidden layer 
...
for seed in range(num_seeds):
    ...
    for num_participants in range(2, 11):  # 2 to 11 participants, life is too short
        ...
        hidden_size = 10 * 3 # As for Prototype 2 here we take 3 HL
        output_size = 10
...
"""
# ResultPlot # We plot all accuracies in a separate file - Accuracy Plots
